...
namespace PTAWMS.Areas.Attendance.Controllers
{
    ...
    public class OTApproverController : Controller
    {
        ...
        [HttpGet]
        public ActionResult AOTDeptList(int? PayrollPeriodID)
        {
            ...
            if (deptIds.Count == 1)
                return RedirectToAction("AOTEmpList", new { DeptID = deptIds.First(), PayrollPeriodID = prp.PID });
            else
            {
        ...
        public ActionResult AOTDeptList()
        {
        ...
        [HttpGet]
        public ActionResult AOTDetailList(int? EmpID, int? PayrollPeriodID)
        {
            ViewUserEmp LoggedInUser = Session["LoggedUser"] as ViewUserEmp;
